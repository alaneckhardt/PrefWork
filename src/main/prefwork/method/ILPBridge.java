...
public class ILPBridge implements InductiveMethod {
	...
	private String getResult(BufferedReader stdOut, BufferedReader stdErr)
			throws IOException {
		String line = null;
		while (true) {
			if (stdOut.ready()) {
	...
	private void read(BufferedReader stdOut, BufferedReader stdErr,
			String endLine) throws IOException {
		while (true) {
	...
	public Double classifyRecord(List<Object> record, Integer targetAttribute) {
		try {
			Process p = Runtime.getRuntime().exec(
					"c:\\F\\devel\\ilp\\SWI\\bin\\plcon.exe", null,
					new File("c:\\install\\ilp\\"));
			/*
			 * Process pp = now .exec("c:\\F\\devel\\ilp\\SWI\\bin\\plcon.exe");
			 */
			/*
			 * Process pp = now .exec("c:\\F\\devel\\ilp\\SWI\\bin\\plcon.exe",
			 * "c:\\install\\ilp\\aleph.pl"});
			 */
			// consult('c:/install/ilp/aleph.pl').
			...
			BufferedReader stdErr = new BufferedReader(new InputStreamReader(p
					.getErrorStream()));
			PrintStream stdIn = new PrintStream(new BufferedOutputStream(p
			...
			stdIn.flush();
			@SuppressWarnings("unused")
			String res = getResult(stdOut, stdErr);
